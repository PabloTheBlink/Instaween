...
for(let i=0;i<maxLength;i++){const originalChild=originalChildren[i];const updatedChild=updatedChildren[i];if(!originalChild&&updatedChild){const newElement=updatedChild.cloneNode(!0);if(newElement.tagName==="IMG"){newElement.setAttribute("loading","lazy");const src=newElement.getAttribute("src");newElement.removeAttribute("src");requestIdleCallback(()=>{const image=new Image();image.src=src;image.onload=()=>{newElement.setAttribute("src",src)}})}
container.appendChild(newElement)}else if(originalChild&&!updatedChild){originalChild.remove()}else if(originalChild&&updatedChild){if(originalChild.nodeType===Node.TEXT_NODE&&updatedChild.nodeType===Node.TEXT_NODE){if(originalChild.textContent.replaceAll(/\s/g,"")!==updatedChild.textContent.replaceAll(/\s/g,"")){log(uuid,"Reemplazando texto",originalChild.textContent,updatedChild.textContent);originalChild.textContent=updatedChild.textContent}}else{if(originalChild.tagName===updatedChild.tagName){for(let attr of originalChild.attributes){if(attr.name==UUID_ATTRIBUTE)continue;if(updatedChild.hasAttribute(attr.name))continue;originalChild.removeAttribute(attr.name)}
